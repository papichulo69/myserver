...
PacketHandler.prototype.handshake_onCompleted = function (protocol, key) {
    ...
    this.socket.sendPacket(new Packet.SetBorder(this.socket.playerTracker, this.gameServer.border, this.gameServer.config.serverGamemode, "JhairTM-YT " + this.gameServer.version));
    // Send welcome message
    this.gameServer.sendChatMessage(null, this.socket.playerTracker, "JhairTM-YT " + this.gameServer.version);
    ...
    if (this.gameServer.config.serverChat == 0)
        this.gameServer.sendChatMessage(null, this.socket.playerTracker, "El chat ha sido desactivado en este cliente!.");
    if (this.protocol < 4)
